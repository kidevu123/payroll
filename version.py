...
VERSION_HISTORY = [
    ...
    },
    {
        ...
        "date": "2026-04-25",
        "changes": [
            "NGTeco: Download menu — do not use [role=menuitem].first (many MUI items are in closed",
            "menus). Scan get_by_role(menuitem) for is_visible(), else ul[role=menu].last + force."
        ]
    },
    {
        "version": "9.5.12",
        "date": "2026-04-25",
        "changes": [
            "NGTeco timecard download: after Download click, wait for [role=menuitem] to appear,",
            "then open csv using anchored ^csv$ (plain export first). Fixes timeout on get_by_text(",
            "csv with tz) when that option is not shown."
        ]
    },
    {
        "version": "9.5.11",
    ...
    },
    {
        ...
        "date": "2026-04-25",
        "changes": [
            "NGTeco: timecard _timecard_set_date_range — MUI read-only inputs, aria-label, placeholders,",
            "HTML5 type=date; get_by_role Select all rows; env NGTECO_TIMECARD_START/_END_SELECTOR.",
            "Clearer error where uploads/ngteco_date_hunt.png lives on the server (e.g. /opt/payroll/uploads/)."
        ]
    },
    {
        "version": "9.5.9",
